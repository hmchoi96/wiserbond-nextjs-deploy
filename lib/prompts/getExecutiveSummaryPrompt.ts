export type ExecutiveSummaryParams = {
  ...
  supportSummary?: string; // ✅ optional fallback
  is_pro?: boolean;
};
...
export function getExecutiveSummaryPrompt(params: ExecutiveSummaryParams): string {
  ...
  const interpretation_fallback = interpretation.trim().length > 0
    ? interpretation
    : (supportSummary.trim().length > 0
        ? `Note: Strategic interpretation was not available. Below is a fallback summary from supplemental research.\n\n${supportSummary}`
        : "[No interpretation or fallback insight available]");
